...
1. `.github/workflows/ci.yml` runs every deterministic pull request check: branch naming and the frontend dependency installation/build.
2. `.github/workflows/ai-review-after-ci.yml` runs from the default branch after `Frontend PR Quality and Security` completes. Its job exists only when the completed run is a successful pull request run with resolved PR metadata.
...
```bash
pytest -q tests/test_ai_review_workflows.py tests/test_github_comment.py tests/test_github_review.py tests/test_openrouter_provider.py tests/test_review_runner.py
python -m pip check
```
...
3. Failing branch-name or frontend build validation leaves the AI workflow unstarted and does not consume provider quota.
