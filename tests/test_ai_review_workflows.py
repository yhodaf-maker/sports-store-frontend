...
ORCHESTRATOR = (ROOT / ".github/workflows/ai-review-after-ci.yml").read_text(encoding="utf-8")
REUSABLE = (ROOT / ".github/workflows/reusable-ai-review.yml").read_text(encoding="utf-8")


def test_all_repository_pr_checks_are_in_authoritative_ci_workflow():
    for required in ("Branch name validation", "Frontend build validation"):
...
def test_orchestrator_only_invokes_review_after_successful_pr_ci():
    assert 'workflows: ["Frontend PR Quality and Security"]' in ORCHESTRATOR
...
def test_reusable_workflow_is_least_privilege_and_data_only():
    for permission in ("actions: read", "contents: read", "pull-requests: write"):
        assert permission in REUSABLE
    for forbidden in ("id-token: write", "contents: write", "packages: write", "security-events: write"):
    ...
    assert "requirements-review.txt" in REUSABLE
...
def test_reusable_workflow_has_pr_concurrency_and_advisory_outputs():
    assert "group: ai-review-${{ inputs.repository }}-${{ inputs.pull_request_number }}" in REUSABLE
...
def test_failed_ci_path_cannot_consume_openrouter_quota():
    success_gate = ORCHESTRATOR.index("github.event.workflow_run.conclusion == 'success'")
